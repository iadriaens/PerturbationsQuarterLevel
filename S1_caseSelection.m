...
% save statements
save('D1_PT_new.mat','PT_all','PT_perc','PT_zero','settings1')
save('D2_PT2_new.mat','PT_all2','PT_perc2','PT_zero2','settings2')
save('D3_Mod_new.mat','model')
...
CLMAS.Vanhullebus = CLMAS.Vereenooghe;
CLMAS.DeBrabander = CLMAS.Debrabander;
CLMAS = rmfield(CLMAS,{'Dewulf','Vereenooghe','Debrabander'});
CLMAS = rmfield(CLMAS,{'ZHunt','ZBentley','ZHartley','ZKelmireGrange','ZCrossley'});
CLMAS = orderfields(CLMAS);
...
% id of HBH = add 'BE '
for i = 1:height(id.Hooibeekhoeve)
    ids{i,1} = ['BE ' num2str(id.Hooibeekhoeve.OfficialRegNo(i))];
end
id.Hooibeekhoeve = removevars(id.Hooibeekhoeve,'OfficialRegNo');
id.Hooibeekhoeve.OfficialRegNo = ids;
...
for i = 1:length(farms)
    ...
    CLMAS.(farms{i}).Lac(:,1) = NaN; % set NaN
    ...
    for j = 1:height(CLMAS.(farms{i}))
        ...
        ind = find(contains(id.(farms{i}).OfficialRegNo,CLMAS.(farms{i}).OffRegNo(j)) == 1 & ...
                   ...
                   datenum(id.(farms{i}).Calving) < datenum(CLMAS.(farms{i}).Date(j)));
        ...
        % match cases
        if ~isempty(ind)
            CLMAS.(farms{i}).B_ID(j) = id.(farms{i}).B_ID(ind(end));
            CLMAS.(farms{i}).Lac(j) = id.(farms{i}).Lac(ind(end));
        end
    end
    ...
    SUM.UniqueCows(i,1) = length(unique(CLMAS.(farms{i}).B_ID(~isnan(CLMAS.(farms{i}).B_ID))));
    SUM.UniqueLac(i,1) = size(unique([CLMAS.(farms{i}).B_ID(~isnan(CLMAS.(farms{i}).B_ID)) CLMAS.(farms{i}).Lac(~isnan(CLMAS.(farms{i}).B_ID))],'rows'),1);
...
SUM.CasesPerCow(:,1) = SUM.CasesMatched./SUM.UniqueCows;
SUM.CasesPerLac(:,1) = SUM.CasesMatched./SUM.UniqueLac;
...
save('D9_TR_cases_new.mat','CLMAS','SUM')
...
for j = 1:length(farms)
    ...
    % summarize cowlac
    cowlac = array2table(unique([moddataDAY.(farms{j}).B_ID,moddataDAY.(farms{j}).Lac],'rows'),'VariableNames',{'B_ID','Lac'});
...
%         doubleindex=find(diff(set.DIM) <0);
%         if ~isempty(doubleindex)
%             set.DIM(doubleindex+1:end) = [];
%             set.MY(doubleindex+1:end) = [];
%             set.MOD(doubleindex+1:end) = [];
%             ProblemCow = [ProblemCow ; j cowlac.B_ID{i} cowlac.Lac{i}];
%         end
...
settings1 = [80,5];
settings2 = [70,5];
save('D5_PTday_new.mat','PT_zeroDAY','settings1')
save('D6_PTday2_new.mat','PT_zeroDAY2','settings2')
save('D7_Mod_new.mat','modelDAY')
...
%% summary day and milk dataset over farms
% first select which farms are included

load('D9_TR_cases_new.mat');

disp(['No of farms = ' num2str(sum(SUM.PercMatched ~= 0))])
disp(['Average no of cases/farm = ' num2str(mean(SUM.CasesMatched)) ' +/- ' num2str(std(SUM.CasesMatched))])
disp(['min = ' num2str(min(SUM.CasesMatched)) ', max = ' num2str(max(SUM.CasesMatched))])
disp(['No of cases = ' num2str(sum(SUM.CasesMatched))]);
disp(['% matched per farm ' num2str(mean(SUM.PercMatched)) ' +/- ' num2str(std(SUM.PercMatched))])
disp(['Unique cows per farm = ' num2str(mean(SUM.UniqueCows)) '+/-' num2str(std(SUM.UniqueCows))])
disp(['min = ' num2str(min(SUM.UniqueCows)) ', max = ' num2str(max(SUM.UniqueCows))])
disp(['No lac per farm = ' num2str(mean(SUM.UniqueLac)) '+/-' num2str(std(SUM.UniqueLac))])
disp(['min = ' num2str(min(SUM.UniqueLac)) ', max = ' num2str(max(SUM.UniqueLac))])
disp(['No of cows = ' num2str(sum(SUM.UniqueCows))])
disp(['No of lac = ' num2str(sum(SUM.UniqueLac))])
disp(['Average per cow ' num2str(mean(SUM.CasesPerCow)) '+/-' num2str(std(SUM.CasesPerCow))])
disp(['Average per Lac ' num2str(mean(SUM.CasesPerLac)) '+/-' num2str(std(SUM.CasesPerLac))])


farms = fieldnames(CLMAS);
for i = 1:length(CLMAS)
